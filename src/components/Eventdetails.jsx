    ...
    <div>
      ...
      <main className="min-h-screen flex flex-col items-center justify-center px-6 md:px-12 mt-16 relative ">
        ...
        {loading ? (
          ...
          <motion.div
            className="flex flex-col md:flex-row items-center bg-gradient-to-br from-white to-gray-100 rounded-xl shadow-lg overflow-hidden w-full max-w-6xl mb-40 transition-transform transform hover:scale-105 lexend border border-black"
          ...
          >
            {/* Event Image */}
            <div className="w-full md:w-1/2 h-64 md:h-auto relative p-3">
              <img
            ...
            {/* Event Info */}
            <div className="flex flex-col items-center md:items-start w-full md:w-1/2 p-6 space-y-4 h-full justify-around  ">
              {/* Date */}
              ...
              {/* Time */}
              <p className="text-lg text-gray-700">
                <strong>Time:</strong> {formatTime(event?.eventTimings.from)} - {formatTime(event?.eventTimings.to)}
              </p>
              ...
              {/* Event Fees */}
              <p className="text-lg text-gray-700">
                <strong>Event Fees:</strong> ₹{event?.eventFees}
              </p>

              {/* Conditionally Render Notes */}
              {event?.eventNote && (
                <p className="text-lg text-gray-700">
                  <strong>Notes:</strong> {event?.eventNote}
                </p>
              )}
              ...
              <div className="mt-auto w-full">
                <button
                  ...
                  className="bg-gradient-to-r from-purple-600 to-indigo-800 hover:from-purple-700 hover:to-indigo-900 text-white font-bold w-full px-6 py-3 rounded-lg shadow-lg transform transition duration-300 hover:scale-105"
                >
                  Register Now <FaArrowRight className="ml-2 inline" />
                </button>
              </div>
            </div>
        ...
        <div className="flex justify-center items-center font-sans relative z-50">
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-1 gap-8 w-full">
            <Coordinator name={event?.eventCoordinator.name} number={event?.eventCoordinator.number} />
          </div>
        </div>
